...
export function joinPath(dir: string, name: string): string {
  ...
  //
  // ⚠️ 要收掉的不只是**結尾**的斜線，中間的也要。原本只去尾，於是
  // `joinPath("a//b", "x")` 會產生 `"a//b/x"` —— property test
  // 「結果永遠不含連續斜線」抽到這個反例才發現。
  // 內部的 `//` 進得來（localStorage 存的路徑、手改的網址），
  // 而後端的 `Path::components` 雖然會摺疊它，那個路徑仍然會原封不動
  // 出現在網址與畫面上。
  // 第二個 replace 用 `/\/$/` 而不是 `/\/+$/`：前一個已經把所有連續斜線
  // 收成單一個，結尾最多只剩一個。（變異測試指出 `+` 在這裡打不死。）
