      ...
      <AnimatePresence mode="wait">
        {step === "config" ? (
          ...
          >
            ...
            {/* Max Files Count */}
            <div className="space-y-3">
              <Label className="text-sm font-medium text-zinc-300">檔案數量限制（選填）</Label>
              <Input
                type="number"
                value={maxFiles ?? ""}
 ...
 * 對外的殼：只負責 `<Dialog>` 的開關。
 *
 * ⚠️ body 只在開啟時掛載——每次開啟都是一次全新的 mount，所有 `useState`
 *    自動回到初始值。原本是在 effect 裡「開啟時逐一 setXxx 重設」，那種寫法
 *    會多跑一次 render，而且日後多一個欄位卻忘了重設不會有任何人發現。
 */
export function UploadLinkDialog({ isOpen, onClose, ...rest }: UploadLinkDialogProps) {
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      {isOpen && <UploadLinkDialogBody onClose={onClose} {...rest} />}
    </Dialog>
  );
}
