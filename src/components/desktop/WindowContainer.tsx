...
const getDockPosition = (appType: AppType) => {
  ...
  if (index === -1)
    return { x: globalThis.window.innerWidth / 2, y: globalThis.window.innerHeight };
  ...
  return (
    <motion.div
      ...
      onDragEnd={(_e, info) => {
        ...
        const isSnapping =
          info.point.y < 50 || info.point.x < 50 || info.point.x > screenWidth - 50;
      ...
      className={cn(
        ...
        useWindowStore.getState().activeWindowId === window.id
    ...
    >
            ...
            const onPointerMove = (moveEvent: PointerEvent) => {
              if (moveEvent.clientY - startY > 10) {
                ...
                const onContinueDragUp = (e: PointerEvent) => {
                  setIsDragging(false);
                  const event = new CustomEvent("window-drag-end", {
                    detail: { x: e.clientX, y: e.clientY, windowId: window.id },
                  });
                  globalThis.window.dispatchEvent(event);
                  ...
                  // Updated threshold to 50
                  const isSnapping =
                    e.clientY < 50 || e.clientX < 50 || e.clientX > screenWidth - 50;
      ...
      >
        ...
        <div className="text-sm font-medium text-black/80 dark:text-white/80">{window.title}</div>
...
export const WindowContainer = () => {
  ...
  const [previewState, setPreviewState] = React.useState<"none" | "maximize" | "left" | "right">(
    "none",
  );
